...
describe("ColorRegistry", function ()
  ...
  -- -------------------------------------------------------------------
  describe("get_flattened_colors_for_research", function ()
    it("returns flattened array and color count for registered ingredients", function ()
      local r = ColorRegistry.new()
      r:set_ingredient_color("automation-science-pack", { 0.91, 0.16, 0.20 })
      ...
      assert.are.equal(0.91, flat[1])
      assert.are.equal(0.16, flat[2])
      assert.are.equal(0.20, flat[3])
      assert.are.equal(0.29, flat[4])
      assert.are.equal(0.97, flat[5])
      assert.are.equal(0.31, flat[6])
    end)
    ...
    it("skips unregistered ingredients", function ()
      ...
      assert.are.equal(3, #flat)
      assert.are.equal(0.91, flat[1])
    end)

    it("returns default color when no ingredient is registered", function ()
      local r = ColorRegistry.new()
      ...
      assert.are.equal(1, n)
      assert.are.equal(3, #flat)
      assert.are.equal(dc[1], flat[1])
      assert.are.equal(dc[2], flat[2])
      assert.are.equal(dc[3], flat[3])
    end)

    it("returns default color for a technology with no ingredients", function ()
      local r = ColorRegistry.new()
    ...
    it("brightness scales all components uniformly", function ()
      local r = ColorRegistry.new()
      r:set_ingredient_color("custom-pack", { 1.0, 0.0, 0.5 })
      local tech = make_tech("custom-pack")
      local flat, n = r:get_flattened_colors_for_research(tech, 1.0, 0.5)
      assert.are.equal(1, n)
      assert.are.equal(0.5, flat[1])
      assert.are.equal(0.0, flat[2])
      assert.are.equal(0.25, flat[3])
    end)

    it("saturation=0 makes all components equal to luminance", function ()
      local r = ColorRegistry.new()
      r:set_ingredient_color("custom-pack", { 1.0, 0.0, 0.0 })
      local tech = make_tech("custom-pack")
      local flat = r:get_flattened_colors_for_research(tech, 0.0, 1.0)
      local lum = 0.299 * 1.0 + 0.587 * 0.0 + 0.114 * 0.0
      assert.are.equal(lum, flat[1])
      assert.are.equal(lum, flat[2])
      assert.are.equal(lum, flat[3])
    end)

    it("saturation blends color toward gray", function ()
      local r = ColorRegistry.new()
      r:set_ingredient_color("custom-pack", { 1.0, 0.0, 0.0 })
      local tech = make_tech("custom-pack")
      local flat = r:get_flattened_colors_for_research(tech, 0.5, 1.0)
      local lum = 0.299 * 1.0 + 0.587 * 0.0 + 0.114 * 0.0
      assert.are.equal(lum + (1.0 - lum) * 0.5, flat[1])
      assert.are.equal(lum + (0.0 - lum) * 0.5, flat[2])
      assert.are.equal(lum + (0.0 - lum) * 0.5, flat[3])
    end)

    it("applies saturation and brightness to multiple ingredients independently", function ()
      local r = ColorRegistry.new()
      ...
      assert.are.equal(2, n)
      assert.are.equal(0.5, flat[1])
      assert.are.equal(0.0, flat[4])
      assert.are.equal(0.5, flat[5])
    end)

    it("applies brightness to default color when no ingredients match", function ()
      local r = ColorRegistry.new()
      local tech = make_tech("unknown-pack")
      local dc = ColorRegistry.default_research_color
      local flat = r:get_flattened_colors_for_research(tech, 1.0, 0.5)
      assert.are.equal(dc[1] * 0.5, flat[1])
      assert.are.equal(dc[2] * 0.5, flat[2])
      assert.are.equal(dc[3] * 0.5, flat[3])
    end)

    it("applies saturation to default color when no ingredients match", function ()
      local r = ColorRegistry.new()
      local tech = make_tech("unknown-pack")
      local dc = ColorRegistry.default_research_color
      local lum = 0.299 * dc[1] + 0.587 * dc[2] + 0.114 * dc[3]
      local flat = r:get_flattened_colors_for_research(tech, 0.0, 1.0)
      assert.are.equal(lum, flat[1])
      assert.are.equal(lum, flat[2])
      assert.are.equal(lum, flat[3])
    ...
    it("defaults to saturation=1.0 and brightness=1.0 when omitted", function ()
      local r = ColorRegistry.new()
      r:set_ingredient_color("custom-pack", { 0.5, 0.5, 0.5 })
      local tech = make_tech("custom-pack")
      local flat = r:get_flattened_colors_for_research(tech)
      assert.are.equal(0.5, flat[1])
  ...
  describe("get_colors_for_research", function ()
    ...
    describe("with saturation and brightness", function ()
      ...
      it("brightness scales all components uniformly", function ()
        ...
        r:set_ingredient_color("custom-pack", { 1.0, 0.0, 0.5 })
        local tech = make_tech("custom-pack")
        local colors = r:get_colors_for_research(tech, 1.0, 0.5)
        assert.are.equal(1, #colors)
        assert.are.equal(0.5, colors[1][1])
      ...
      it("saturation=0 makes all components equal to luminance", function ()
        ...
        r:set_ingredient_color("custom-pack", { 1.0, 0.0, 0.0 })
        local tech = make_tech("custom-pack")
        local colors = r:get_colors_for_research(tech, 0.0, 1.0)
        local lum = 0.299 * 1.0 + 0.587 * 0.0 + 0.114 * 0.0
        assert.are.equal(lum, colors[1][1])
        assert.are.equal(lum, colors[1][2])
        assert.are.equal(lum, colors[1][3])
      end)
      ...
      it("saturation blends color toward gray", function ()
        ...
        r:set_ingredient_color("custom-pack", { 1.0, 0.0, 0.0 })
        local tech = make_tech("custom-pack")
        local colors = r:get_colors_for_research(tech, 0.5, 1.0)
        local lum = 0.299 * 1.0 + 0.587 * 0.0 + 0.114 * 0.0
        assert.are.equal(lum + (1.0 - lum) * 0.5, colors[1][1])
        assert.are.equal(lum + (0.0 - lum) * 0.5, colors[1][2])
        assert.are.equal(lum + (0.0 - lum) * 0.5, colors[1][3])
      end)

      it("applies saturation and brightness to all ingredients independently", function ()
        local r = ColorRegistry.new()
        r:set_ingredient_color("pack-a", { 1.0, 0.0, 0.0 })
        r:set_ingredient_color("pack-b", { 0.0, 1.0, 0.0 })
        local tech = make_tech("pack-a", "pack-b")
      ...
      it("applies brightness to default colors when no ingredients match", function ()
        local r = ColorRegistry.new()
        local tech = make_tech("unknown-pack")
        local dc = ColorRegistry.default_research_color
        local colors = r:get_colors_for_research(tech, 1.0, 0.5)
    ...
    describe("with color_prefixes", function ()
      ...
      it("applies brightness to prefix-resolved color", function ()
        set_prototype_data(
    ...
    describe("with color_suffixes", function ()
      ...
      it("applies brightness to suffix-resolved color", function ()
        set_prototype_data(
          { ["automation-science-pack"] = { 1.0, 0.0, 0.5 } },
          nil,
          { "-compressed" }
