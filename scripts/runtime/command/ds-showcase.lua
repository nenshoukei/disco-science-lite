...
commands.add_command("ds-showcase", "Set up showcase of all labs.", function (event)
  ...
  for _, proto in pairs(prototypes.get_entity_filtered({ { filter = "type", type = "lab" } })) do
    ...
    if not lab_priorities[proto.name] then
      ...
      lab_priorities[proto.name] = (researchable and 1000 or 0) + table_size(proto.lab_inputs or {})
    end
  end
...
commands.add_command("ds-showcase2", "Set up showcase of vanilla labs in mega-base. Usage: /ds-showcase2 [tile_size:integer=320]", function (event)
  if not event.player_index then return end

  local player = game.get_player(event.player_index)
  if not player then return end

  local tile_size = event.parameter and event.parameter ~= "" and tonumber(event.parameter) or 320
  if tile_size < 1 then return end
  ...
  local width = math.max(proto.tile_width, 1)
  local height = math.max(proto.tile_height, 1)
  while y < tile_size do
    while x < tile_size do
      local lab = surface.create_entity({
        name = proto.name,
        position = { x = x + width / 2, y = y + height / 2 },
        force = player.force,
