...
local function is_valid_color(color)
  return type(color) == "table"
    and ((type(color[1]) == "number" and type(color[2]) == "number" and type(color[3]) == "number")
      or (type(color.r) == "number" and type(color.g) == "number" and type(color.b) == "number"))
end
...
local function is_valid_color_array(colors)
  ...
  for i = 1, n_colors do
    if not colors[i] or not is_valid_color(colors[i]) then
      is_all_valid = false
      break
    end
  end
...
RemoteInterface.functions = {
  setLabScale = function (lab_name, scale)
  ...
  setIngredientColor = function (item_name, color)
    assert(
      type(item_name) == "string" and item_name ~= "",
      "DiscoScience.setIngredientColor: item_name must be a non-empty string"
    )
    assert(
      is_valid_color(color) or is_valid_color_array(color),
  ...
  getIngredientColor = function (item_name)
  ...
  getIngredientColors = function (item_name)
    if not color_registry then return end
    assert(
      type(item_name) == "string" and item_name ~= "",
