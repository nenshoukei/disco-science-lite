...
--- @type ModSupport
local mod = {}
if not (mods["omnimatter_compression"] or mods["omnimatter_science"] or mods["omnimatter_energy"]) then return mod end
...
mod.on_data = function ()
  if mods["omnimatter_compression"] then
    ...
    if level and level.value then
      ...
      local max_level = level.value --[[@as integer]]
      for i = 1, math.min(max_level, #COMPRESS_LEVEL) do
    ...
    data:extend({
      {
        ...
        shift = light.shift,
        scale = 194 / (light.width or 194) * 0.5,
      },
    })
  end)
end
