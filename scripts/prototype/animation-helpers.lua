...
AnimationHelpers.__index = AnimationHelpers
...
function AnimationHelpers.copy_geometric_properties(source)
  local result = {}
...
--- @class AnimationWithHelpers : AnimationHelpers, data.Animation

--- Use AnimationHelpers for modifying lab.on_animation.
---
--- Callback is called with on_animation whose metatable is set to AnimationHelpers, so
--- helper functions are callable like `on_animation:remove_layers("target-file-name.png")`.
---
--- If target lab is not defined or lab.on_animation is nil, callback is not called.
---
--- @param lab_name string
--- @param callback fun(on_animation: AnimationWithHelpers, lab: data.LabPrototype)
function AnimationHelpers.modify_on_animation(lab_name, callback)
  ...
  local original_metatable = getmetatable(on_animation)
  setmetatable(on_animation, AnimationHelpers)
  local success, err = pcall(callback, on_animation, lab)
  setmetatable(on_animation, original_metatable)
  if not success then
    error(err)
  end
end
...
function AnimationHelpers.get_layer(animation, target_filename)
...
function AnimationHelpers.remove_layer(animation, target_filename)
  local layers = animation.layers
  if not layers then return nil end

  local layer, index = AnimationHelpers.get_layer(animation, target_filename)
  if layer then
...
function AnimationHelpers.replace_filename(animation, old_filename, new_filename)
  ...
  if layers then
    for i = 1, #layers do
      AnimationHelpers.replace_filename(layers[i], old_filename, new_filename)
    end
  end
end
...
--- @param override_props data.Animation?
function AnimationHelpers.insert_mask_layer(animation, target_filename, mask_filename, override_props)
  local layers = animation.layers
  ...
  local new_layer = AnimationHelpers.copy_geometric_properties(source_layer)
...
--- Does nothing if trigger is not found or if `animation.layers` is nil.
---
--- @param animation data.Animation
--- @param frame_index integer? 1-based frame index to freeze at. (Default: `1`)
function AnimationHelpers.freeze_animation(animation, frame_index)
  local layers = animation.layers
  if not layers then return end
...
---
--- @param animation data.Animation
function AnimationHelpers.apply_lab_modifications(animation)
  AnimationHelpers.remove_layer(animation, "__base__/graphics/entity/lab/lab-light.png")

  --- Support Factorio HD Age
  if mods["factorio_hd_age_base_game_production"] then
    AnimationHelpers.remove_layer(animation, "__factorio_hd_age_base_game_production__/data/base/graphics/entity/lab/lab-light.png")
  end
...
---
--- @param animation data.Animation
function AnimationHelpers.apply_biolab_modifications(animation)
  AnimationHelpers.remove_layer(animation, "__space-age__/graphics/entity/biolab/biolab-lights.png")

  --- Support Factorio HD Age
  if mods["factorio_hd_age_space_age_production"] then
    AnimationHelpers.remove_layer(animation, "__factorio_hd_age_space_age_production__/data/space-age/graphics/entity/biolab/biolab-lights.png")
  end
end
